...
| 3 | 腾讯地图 Api | 地址表单地图选点、逆地址解析、关键词搜索、经纬度记录 |
| 4 | Echarts 图形化分析 | 商家端 / 管理端销售、订单、文章等多维度数据可视化 |
| 5 | WebSocket 实时通讯 | Spring Boot WebSocket 实现用户与商家在线沟通 |
| 6 | 物流快递 Api 查询 | 对接阿里云市场快递查询 Api，物流轨迹时间轴展示 |
| 7 | 支付宝沙箱支付 | 下单、支付、定时查单、退款全流程，答辩演示零成本 |
| 8 | 优惠券营销与会员积分 | 优惠券创建 / 领取 / 使用 + 会员等级折扣体系 |
...
| 图表 / 地图 / 通信 | Echarts、腾讯地图 Api、WebSocket |
...
> 为防止截图被盗用，本文档仅展示 10 张核心页面，完整界面可拉取项目运行查看。所有截图均采用**相对路径引用**，随仓库 `images/` 目录一起提交，GitHub / Gitee 均可直接渲染，不会被外链防盗链拦截。
...
### 5. 腾讯地图 Api 选点
店铺地址、收货地址等表单接入腾讯地图 Api，支持地图选点、逆地址解析、关键词搜索和经纬度记录，提高地址录入准确性。
...
> 💡 提示：支付宝沙箱支付、快递查询 Api、腾讯地图 Api、DeepSeek 大模型等第三方服务需要自行申请相关账号 / 密钥，并按项目配置说明替换。

## 📌 说明

- 本项目仅供学习交流使用；
- 如需二次开发，建议先阅读"核心实现说明"部分，了解推荐算法、支付流程与 AI 问答的实现思路。
